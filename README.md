...
Available in **`online.html`** (which also runs local hot-seat and vs-AI, so local *and* online games are covered). When a game ends, click **⬇ Download this game** to save a `circuit-replay-*.json`. Later, from the start screen use **📂 Load game / watch replay**, pick that file, and step through it turn-by-turn with **◀ Prev / ▶ Play / Next ▶** (and **Exit replay**). The board is read-only during a replay. (In online games the host records and shares the replay with the guest, so both can download it.)

## Online multiplayer (two humans, via the server)

Online play uses **`online.html`** (your local `index.html` is unchanged). Players connect through the Node server, which relays moves by a **4-character room code**: one player **Creates** a game (gets a code), the other **Joins** with it. Human vs human; the AI opponents stay a local feature.

### Try it locally
1. `npm install`, then `node server.js`.
2. Open **http://localhost:8787/online.html** in two browser windows.
...
4. When live, share **https://<your-app>.onrender.com/online.html** — one player Creates, the other Joins with the code.
...
| `circuit.html` | Identical copy / working source. You can delete this if you only want one. |
| `DESIGN.md` | Human-readable summary of the rules and design decisions. |
| `CIRCUIT_GDD_v0.4.docx` | The full Game Design Document with the complete change history. |
| `CHANGELOG.md` | Version-by-version summary of how the design evolved. |
| `assets/` | Screenshot used in this README. |
| `online.html` | The online-multiplayer version (Create/Join by 4-char code). |
...
- **Two ways to play multiplayer:** hot-seat (same screen, `index.html`) and online (two devices via the server, `online.html`). Online needs the Node server running (locally or on Render) — GitHub Pages is static-only, so the online page won't work when served from Pages.
- Built as a sing