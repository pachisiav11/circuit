...
app.use(express.static(__dirname));           // serve index.html / online.html etc.
...
io.on('connection', (socket) => {
  ...
  socket.on('create', (ack) => {
    code = genCode(); role = 'host'; rooms[code] = { host: socket.id, guest: null };
    socket.join(code); if (ack) ack({ ok: true, code, role });
  });

  socket.on('join', (c, ack) => {
    c = String(c || '').toUpperCase(); const r = rooms[c];
    if (!r) { if (ack) ack({ ok: false, error: 'No game with that code.' }); return; }
    if (r.guest) { if (ack) ack({ ok: false, error: 'That game is already full.' }); return; }
    r.guest = socket.id; code = c; role = 'guest'; socket.join(c);
    if (ack) ack({ ok: true, code: c, role });
    io.to(r.host).emit('opponentJoined');          // tell host to start
  });
